...
- 간격(5분·10분)은 의존성 마진. collect/brief가 지연돼도 StartWhenAvailable로 따라잡되, 같은 날 순서는 유지된다.
...
```cmd
schtasks /Create /TN "rss-feed collect 0600" /TR "wsl.exe -d Ubuntu --cd /mnt/d/Project/ai-make/rss-feed -- npm run collect" /SC DAILY /ST 06:00 /F
schtasks /Create /TN "rss-feed brief 0605"   /TR "wsl.exe -d Ubuntu --cd /mnt/d/Project/ai-make/rss-feed -- npm run brief"   /SC DAILY /ST 06:05 /F
schtasks /Create /TN "rss-feed daily 0615"   /TR "wsl.exe -d Ubuntu --cd /mnt/d/Project/ai-make/rss-feed -- npm run daily"   /SC DAILY /ST 06:15 /F
```

(WSL 배포판이 Ubuntu가 아니면 `-d <이름>` 수정. `wsl -l` 로 확인.)

등록 확인: `schtasks /Query /TN "rss-feed collect 0600" /V /FO LIST`
시각만 바꾸려면 재등록(`/F`)하거나 `schtasks /Change /TN "..." /ST HH:MM`.

## 등록 방법 B — XML 임포트

`scheduler/rss-collect-0600.xml`, `scheduler/rss-brief-0605.xml`, `scheduler/rss-daily-0615.xml` 를 작업 스케줄러(`taskschd.msc`) → **작업 가져오기**로 각각 임포트. (StartBoundary 날짜는 과거여도 매일 트리거에는 무관 — 시각만 유효.)

XML은 UTF-8 인코딩. Task Scheduler 임포트가 인코딩으로 거부하면 인코딩 문제 없는 **방법 A(schtasks)** 를 사용한다(권장).

## 등록 방법 C — GUI 수동

`taskschd.msc` → 기본 작업 만들기 → 트리거 매일 06:00(collect)/06:05(brief)/06:15(daily) → 동작 "프로그램 시작" → 프로그램 `wsl.exe`, 인수 `-d Ubuntu --cd /mnt/d/Project/ai-make/rss-feed -- npm run collect`(또는 `npm run brief` / `npm run daily`) → 속성에서 "AC 전원에서만 실행" 해제 권장.
...
- **PC 꺼짐/절전으로 트리거 누락**: XML/작업 모두 `StartWhenAvailable=true` — 부팅 후 지난 작업을 가능한 한 빨리 실행.
...
`news_*.json → 호재/악재 분석 → briefing_*.md` LLM 단계는 `scripts/brief.ts`(`npm run brief`, 06:05 작업)가 헤드리스 `claude -p`로 자동 수행 → **완전 무인**. 트레이드오프: 매일 claude 토큰 비용, 분석 품질은 모델 의존(기본 Sonnet, `BRIEF_MODEL`로 Opus 상향 가능), claude CLI가 해당 머신에 인증돼 있어야 함. 품질이 중요한 날은 사람이 `briefing_<오늘>.md`를 직접 작성/덮어쓰면 brief가 skip한다.
